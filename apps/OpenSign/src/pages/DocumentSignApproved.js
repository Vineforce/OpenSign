...
const DocumentSignApproved = () => {
  ...
  const currentList = SignApproved?.slice(indexOfFirstDoc, indexOfLastDoc);
  ...
  const fetchDocumentsByApproverId = async (approverId) => {
    try {
      setIsLoader(true);
      const params = { approverId: approverId, documentSignApprovalFlag: 'Approved' }
  ...
  const getPaginationRange = () => {
    ...
    const pages = [];
    const totalPages = Math.ceil(SignApproved.length / recordperPage);
        ...
        <div className="p-2 w-full bg-base-100 text-base-content op-card shadow-lg">
          ...
          <div className="flex flex-row items-center justify-between my-2 mx-3 text-[20px] md:text-[23px]">
            <div className="font-light">
              ...
              <span className="text-xs md:text-[13px] font-normal">
                <Tooltip message="List of document(s) that you have approved and the Signers has started signing the document." />
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-row  justify-between items-center text-xs font-medium">
            <div className="op-join flex flex-wrap items-center p-2">
              {SignApproved.length > recordperPage && (
                <button
...
export default DocumentSignApproved;
