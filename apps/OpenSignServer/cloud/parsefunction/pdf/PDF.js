...
async function sendNotifyMail(doc, signUser, mailProvider) {
  try {
    ...
    if (remaingsign > 1 && doc?.NotifyOnSignatures) {
      ...
      const logo = `<div><img src='https://www.excis.com/assets/images/main-logo.png' height='50' style='padding:20px' /></div>`;
      const opurl = ` <a href=https://www.excis.com target=_blank> here</a>`;
...
async function sendCompletedMail(obj) {
  ...
  const logo =`<div><img src='https://www.excis.com/assets/images/main-logo.png' height='50' style='padding:20px'/></div>`;
  const opurl =` <a href=https://www.excis.com target=_blank> here</a>.</p></div></div></body></html>`;
